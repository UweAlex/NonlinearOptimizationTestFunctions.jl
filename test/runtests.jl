...
@testset "Filter and Properties Tests" begin
	...
	@test length(filter_testfunctions(tf -> has_property(tf, "bounded"))) == 58
    @test length(filter_testfunctions(tf -> has_property(tf, "continuous"))) == 57
    ...
    @test length(filter_testfunctions(tf -> has_property(tf, "differentiable"))) == 55
