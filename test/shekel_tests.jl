...
@testset "Shekel Tests" begin
    ...
    @test shekel(tf.meta[:min_position](n)) ≈ tf.meta[:min_value] atol=1e-3
    @test shekel(tf.meta[:start](n)) ≈ -0.46329209926592196 atol=1e-6
    @test tf.meta[:name] == "shekel"
    @test tf.meta[:start](n) == [2.0, 2.0, 2.0, 2.0]
    @test tf.meta[:min_position](n) ≈ [4.000747838, 4.000592862, 3.999663587, 3.999510115] atol=1e-6
    ...
    @testset "Optimization Tests" begin
        ...
        @test Optim.minimizer(result) ≈ tf.meta[:min_position](n) atol=0.02  # Toleranz erhöht für Robustheit
    end
end