# test/mishra5_tests.jl
using Test, NonlinearOptimizationTestFunctions
@testset "mishra5" begin
    tf = MISHRA5_FUNCTION

    @test tf.meta[:name] == "mishra5"
    ...
    @test f_start ≈ 2.473542615672 atol=1e-3

    min_pos = tf.meta[:min_position]()
    @test min_pos ≈ [-1.98682, -10.0] atol=1e-6
    f_min = tf.f(min_pos)
    ...
    # Extra point
    test_pt = [1.0, 1.0]
    @test tf.f(test_pt) ≈ 7.60163135773 atol=1e-3
end