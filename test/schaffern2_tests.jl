...
@testset "Schaffer N.2 Tests" begin
    ...
    # Function values
    @test schaffern2(tf.meta[:min_position](n)) ≈ tf.meta[:min_value] atol=1e-10
    @test schaffern2(tf.meta[:start](n)) ≈ 0.0 atol=1e-10  # Start is at minimum
    ...
    @testset "Optimization Tests" begin
        ...
        start = tf.meta[:min_position](n) + 0.01 * randn(2)
    ...
    @testset "Boundary Behavior Tests" begin
        ...
        # Because sin² term becomes negligible compared to denominator growth
        large_val = 100.0
