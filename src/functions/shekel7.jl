...
function shekel7(x::AbstractVector{T}) where {T<:Union{Real, ForwardDiff.Dual}}
    ...
    n != 4 && throw(ArgumentError("shekel7 requires exactly 4 dimensions"))  # Dynamischer Fehlertext [RULE_ERROR_TEXT_DYNAMIC]
...
function shekel7_gradient(x::AbstractVector{T}) where {T<:Union{Real, ForwardDiff.Dual}}
    n = length(x)
    n == 0 && throw(ArgumentError("Input vector cannot be empty"))
    n != 4 && throw(ArgumentError("shekel7 requires exactly 4 dimensions"))  # Dynamisch [RULE_ERROR_TEXT_DYNAMIC]
...
const SHEKEL7_FUNCTION = TestFunction(
    ...
    shekel7_gradient,
    Dict(
        :name => "shekel7",  # Dynamisch: "shekel7" [RULE_NAME_CONSISTENCY]
