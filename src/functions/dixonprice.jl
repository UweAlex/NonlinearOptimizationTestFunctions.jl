...
function dixonprice(x::AbstractVector{T}) where {T<:Union{Real, ForwardDiff.Dual}}
    ...
    n < 1 && throw(ArgumentError("dixonprice requires at least 1 dimension"))
...
const DIXONPRICE_FUNCTION = TestFunction(
    ...
    Dict(
        :name => "dixonprice",
        :start => (n::Int) -> begin n < 1 && throw(ArgumentError("dixonprice requires at least 1 dimension")); fill(1.0, n) end,
        :min_position => (n::Int) -> begin n < 1 && throw(ArgumentError("dixonprice requires at least 1 dimension")); [2^(-(2^i - 2) / 2^i) for i in 1:n] end,
        :min_value => (n::Int) -> begin n < 1 && throw(ArgumentError("dixonprice requires at least 1 dimension")); 0.0 end,
        :properties => ["differentiable", "non-convex", "scalable", "unimodal", "bounded", "continuous"],
        :default_n => 2,
        :lb => (n::Int) -> begin n < 1 && throw(ArgumentError("dixonprice requires at least 1 dimension")); fill(-10.0, n) end,
        :ub => (n::Int) -> begin n < 1 && throw(ArgumentError("dixonprice requires at least 1 dimension")); fill(10.0, n) end,
