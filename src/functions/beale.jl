...
Computes the Beale function value at point `x`. Requires dimension n = 2.
Returns `NaN` for inputs containing `NaN`, and `Inf` for inputs containing `Inf`.
"""
function beale(x::AbstractVector{T}) where {T<:Union{Real, ForwardDiff.Dual}}
    length(x) == 2 || throw(ArgumentError("Beale requires exactly 2 dimensions"))
...
const BEALE_FUNCTION = TestFunction(
    ...
    Dict(
        ...
        :description => "Beale function: Multimodal, non-convex function with global minimum at zero, defined for 2 dimensions.",
