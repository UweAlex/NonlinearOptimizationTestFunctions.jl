...
function mvf_shubert(x::AbstractVector{T}) where {T<:Union{Real, ForwardDiff.Dual}}
    ...
    n != 2 && throw(ArgumentError("mvf_shubert requires exactly 2 dimensions"))  # Fixed 2D [RULE_ERROR_TEXT_DYNAMIC]
...
function mvf_shubert_gradient(x::AbstractVector{T}) where {T<:Union{Real, ForwardDiff.Dual}}
    n = length(x)
    n == 0 && throw(ArgumentError("Input vector cannot be empty"))
    n != 2 && throw(ArgumentError("mvf_shubert requires exactly 2 dimensions"))
...
const MVF_SHUBERT_FUNCTION = TestFunction(
    ...
    Dict(
        :name => "mvf_shubert",  # "mvf_shubert" [RULE_NAME_CONSISTENCY]
